"""
Knowledge Base Ingestion Pipeline for Feature 3: RAG-Grounded Roadmap & Resource Orientation
Loads and processes the Tunisian entrepreneurship knowledge base for use in retrieval and recommendation.
"""
...
def load_clean_kb(path: Path) -> List[Dict[str, Any]]:
    ...
    if not path.exists():
        logger.error(f"Clean KB not found at {path}")
        logger.error("       Run kb_validator_transformer.py first.")
        raise FileNotFoundError(f"Clean KB not found at {path}")

    with open(path, "r", encoding="utf-8") as f:
        items = json.load(f)

    logger.info(f"Loaded {len(items)} clean KB items from {path}")
    return items
...
def init_embedding_model(model_name: str):
    ...
    logger.info(f"Model loaded. Dimension: {model.get_embedding_dimension()}")
    return model
...
def init_chromadb(persist_dir: Path, collection_name: str):
    ...
    logger.info(f"Created collection '{collection_name}' at {persist_dir}")
    return collection
...
def ingest_to_chromadb(
    ...
    total = len(items)
    logger.info(f"Ingesting {total} items in batches of {batch_size}...")
    ...
    for i in range(0, total, batch_size):
        ...
        logger.info(f"   [{progress}/{total}] Ingested batch {(i // batch_size) + 1}")

    logger.info(f"Ingestion complete. Collection has {collection.count()} items.")
...
def ingest_to_sqlite(conn: sqlite3.Connection, items: List[Dict[str, Any]]):
    ...
    logger.info(f"Inserting {total} items...")
    ...
    for idx, item in enumerate(items):
        ...
        if (idx + 1) % 20 == 0 or (idx + 1) == total:
            logger.info(f"   [{idx + 1}/{total}] Inserted into SQLite")
    ...
    logger.info(f"Verification: {count} programs in database")
